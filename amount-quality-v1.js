(() => {
  ...
  // Tag only genuinely new log IDs created after a user logging action. Imports and
  // unrelated state writes are left alone, so older entries remain honestly unknown.
  ...
  if(quick && quickCalc && !$('amountQualityControl')){
    ...
    function renderQuick(){
      qsa('[data-amount-choice]',box).forEach(b=>b.classList.toggle('active',b.dataset.amountChoice===quickQuality));
    }
    qsa('[data-amount-choice]',box).forEach(b=>b.addEventListener('click',()=>{quickQuality=b.dataset.amountChoice;renderQuick();}));
    renderQuick();
  }

  // Arm amount quality from the actual user action that creates log entries.
  ...
  function dateKeys(n){
    const a=[];
    for(let i=n-1;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);a.push(d.toISOString().slice(0,10));}
    return a;
  }
  ...
  function renderWeeklyQuality(){
    ...
    if(intel){
      ...
      if(!insight){insight=document.createElement('div');insight.className='insight amount-quality-intel';intel.appendChild(insight);}
      insight.textContent=qualitySentence();
    }
    const card=$('amountQualitySettingsCard');
    if(card){
      const q=qualityStats(7);
      const w=qs('[data-q="weighed"]',card),e=qs('[data-q="estimated"]',card),u=qs('[data-q="unknown"]',card);
      if(w)w.textContent=q.total?q.weighedPct+'%':'—';if(e)e.textContent=q.total?q.estimatedPct+'%':'—';if(u)u.textContent=q.total?q.unknownPct+'%':'—';
    }
  }

  const history=$('tab-history');
  if(history)new MutationObserver(renderWeeklyQuality).observe(history,{childList:true,subtree:true});

  const settings=$('tab-settings');
  if(settings&&!$('amountQualitySettingsCard')){
    const card=document.createElement('section');
    card.id='amountQualitySettingsCard';card.className='card amount-quality-card';
    ...
    const memory=$('personalFoodMemoryCard');if(memory)memory.insertAdjacentElement('afterend',card);else settings.appendChild(card);
  }
  renderWeeklyQuality();
