(() => {
  ...
  style.textContent=`
    ...
    .scanner-sheet[hidden]{display:none!important}.scanner-sheet{position:fixed;inset:0;z-index:2200;background:rgba(10,18,14,.72);display:flex;align-items:flex-end;justify-content:center;padding:14px}
    ...
    .scanner-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin:10px 0}.scanner-action{min-height:46px;border-radius:12px;font-weight:850;padding:10px 12px;touch-action:manipulation}.scanner-action.primary{border:0;background:#1E4235;color:#fff}.scanner-action.secondary{border:1px solid #1E4235;background:#fff;color:#1E4235}.scanner-action.full{grid-column:1/-1}
  ...
  const sheet=document.createElement('div');sheet.className='scanner-sheet';sheet.hidden=true;sheet.setAttribute('role','dialog');sheet.setAttribute('aria-modal','true');sheet.setAttribute('aria-labelledby','scannerTitle');sheet.innerHTML=`
    <div class="scanner-panel">
      <div class="scanner-head"><div><h3 id="scannerTitle">Scan food barcode</h3><p>Use the live camera, or take a barcode photo if iPhone blocks the live stream.</p></div><button type="button" class="scanner-close" id="scannerCloseBtn" aria-label="Close scanner">×</button></div>
      ${standalone&&isiPhone?'<div class="scanner-pwa-note">Installed iPhone web apps can behave differently from Safari with camera permissions. The photo option avoids the live-camera path and is usually the most reliable fallback.</div>':''}
      <div class="scanner-actions">
        <button id="scannerStartLiveBtn" class="scanner-action primary" type="button">Start live camera</button>
        <button id="scannerTakePhotoBtn" class="scanner-action secondary" type="button">Take barcode photo</button>
      ...
      <p class="scanner-help">If live camera permission fails in the Home Screen app, use Take barcode photo or open the same app in Safari. You can always type the barcode manually.</p>
    </div>`;
  document.body.appendChild(sheet);

  const closeBtn=$('scannerCloseBtn'),status=$('scannerStatus'),frame=$('scannerFrame'),photoInput=$('scannerPhotoInput');
  let scanner=null,closing=false,found=false;
  ...
  function loadScannerLibrary(){
    ...
    return new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/html5-qrcode@2.3.8/html5-qrcode.min.js';s.async=true;s.dataset.okelloScannerLib='1';s.onload=()=>{s.dataset.loaded='1';resolve();};s.onerror=reject;document.head.appendChild(s);
    });
  }
  ...
  async function closeScanner(){await stopScanner();sheet.hidden=true;frame.hidden=true;document.body.style.overflow='';scanBtn.focus();}
  function openScanner(){found=false;sheet.hidden=false;frame.hidden=true;document.body.style.overflow='hidden';setStatus('Choose live camera or barcode photo.');}
  ...
  async function onDecoded(text){
    ...
    await stopScanner();
    setTimeout(()=>{sheet.hidden=true;frame.hidden=true;document.body.style.overflow='';lookupBtn.click();input.scrollIntoView({behavior:'smooth',block:'center'});found=false;},260);
  }
  ...
  async function startLive(){
    ...
    if(!window.isSecureContext){setStatus('Camera scanning needs a secure HTTPS page.','bad');return;}
    if(!navigator.mediaDevices?.getUserMedia){setStatus('This browser cannot access the live camera. Use Take barcode photo instead.','bad');return;}
    try{
      // Ask for camera permission directly from this real user tap before awaiting
      // the scanner library. This is more reliable in iPhone standalone mode.
      ...
      const formats=window.Html5QrcodeSupportedFormats?[
        window.Html5QrcodeSupportedFormats.EAN_13,window.Html5QrcodeSupportedFormats.EAN_8,window.Html5QrcodeSupportedFormats.UPC_A,window.Html5QrcodeSupportedFormats.UPC_E,window.Html5QrcodeSupportedFormats.CODE_128,window.Html5QrcodeSupportedFormats.CODE_39,window.Html5QrcodeSupportedFormats.ITF
      ].filter(Boolean):undefined;
      const config={fps:10,qrbox:{width:280,height:120},aspectRatio:1.777};if(formats?.length)config.formatsToSupport=formats;
      await scanner.start({facingMode:{ideal:'environment'}},config,onDecoded,()=>{});
      setStatus('Scanning… keep the barcode inside the box.');
    }catch(err){console.error(err);try{await stopScanner();}catch(_){}setStatus(standalone&&isiPhone?'Live camera could not start in the installed app. Try Take barcode photo below or open in Safari.':'I could not start the camera. Check camera permission or use Take barcode photo.','bad');}
  }
  ...
  $('scannerStartLiveBtn').addEventListener('click',startLive);
  $('scannerTakePhotoBtn').addEventListener('click',()=>photoInput.click());
  ...
  // The sticky hub used to trigger a synthetic click on the Foods-page scanner.
  // In standalone iPhone mode that can lose the user-gesture context needed by
  // camera permission. Capture the real tap and open this sheet directly instead.
  document.addEventListener('click',e=>{
    const hub=e.target?.closest?.('#hubScanBtn');
    if(!hub)return;
    e.preventDefault();e.stopImmediatePropagation();openScanner();
  },true);

  window.OkelloScanner=Object.freeze({version:2,open:openScanner,startLive,close:closeScanner,scanPhoto});
})();