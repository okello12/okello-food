(() => {
  ...
  css.textContent = `
    ...
    .hub-online-loading{padding:13px 14px;color:var(--muted);font-size:.88rem}
  ...
  function selectFood(id){
    ...
    foodSelect.value=id;foodSelect.dispatchEvent(new Event('change',{bubbles:true}));
    clickTab('today');
    setTimeout(()=>{quickAdd.scrollIntoView({behavior:'smooth',block:'start'});$('gramsInput')?.focus({preventScroll:true});},90);
    return true;
  }

  const hub=document.createElement('section');
  hub.className='global-hub';
  hub.innerHTML=`
    <div class="hub-search-row">
      <div class="hub-search-box"><span class="hub-search-icon" aria-hidden="true">⌕</span><input id="globalFoodSearch" type="search" autocomplete="off" placeholder="Search food or packaged product…" aria-label="Search all foods"><button id="hubClearBtn" class="hub-clear" type="button" aria-label="Clear search">×</button></div>
      <button id="hubScanBtn" class="hub-scan" type="button">▣ Scan</button>
    </div>
    <div class="hub-tools">
      <button id="hubAddBtn" class="hub-tool" type="button">＋ Add food</button>
      <button id="hubRecipeBtn" class="hub-tool" type="button">🍲 Recipe</button>
      <button id="hubFoodsBtn" class="hub-tool" type="button">Browse library</button>
      <span id="hubBalance" class="hub-balance">2,300 kcal left</span>
    </div>
    <div id="hubResults" class="hub-results" hidden></div>`;
  mast.insertAdjacentElement('afterend',hub);

  const globalSearch=$('globalFoodSearch'),hubResults=$('hubResults'),clearBtn=$('hubClearBtn');
  let searchTimer=null;

  function localResults(term){
    const q=term.trim().toLowerCase();
    if(!q)return [];
    return optionList().filter(x=>x.label.toLowerCase().includes(q)).slice(0,9);
  }
  function showLocalResults(){
    const q=globalSearch.value.trim();clearBtn.classList.toggle('show',!!q);
    if(!q){hubResults.hidden=true;hubResults.innerHTML='';return;}
    const matches=localResults(q);
    const rows=matches.map(x=>`<button type="button" class="hub-result" data-hub-food="${esc(x.id)}"><span><strong>${esc(x.label)}</strong><small>Use smart portion and log it</small></span><span class="go">›</span></button>`).join('');
    ...
    hubResults.innerHTML=rows+online;
    hubResults.hidden=false;
    hubResults.querySelectorAll('[data-hub-food]').forEach(b=>b.addEventListener('click',()=>{selectFood(b.dataset.hubFood);globalSearch.value='';showLocalResults();}));
    $('hubOnlineBtn')?.addEventListener('click',()=>searchOnline(q));
  }
  ...
  clearBtn.addEventListener('click',()=>{globalSearch.value='';showLocalResults();globalSearch.focus();});
  document.addEventListener('click',e=>{if(!hub.contains(e.target))hubResults.hidden=true;});

  $('hubScanBtn').addEventListener('click',()=>{
    const scan=$('scanBarcodeBtn');
    if(scan)scan.click();
    else{clickTab('foods');setTimeout(()=>$('scanBarcodeBtn')?.click(),80);}
  });
  ...
  const onlineSheet=document.createElement('div');
  onlineSheet.className='online-sheet';onlineSheet.hidden=true;
  onlineSheet.innerHTML=`<div class="online-panel" role="dialog" aria-modal="true" aria-labelledby="onlineTitle"><div class="online-head"><div><p class="eyebrow">ONLINE PRODUCT SEARCH</p><h3 id="onlineTitle">Packaged foods</h3></div><button class="online-close" id="onlineClose" type="button" aria-label="Close">×</button></div><div id="onlineList" class="online-list"></div></div>`;
  document.body.appendChild(onlineSheet);
  $('onlineClose').addEventListener('click',()=>onlineSheet.hidden=true);
  onlineSheet.addEventListener('click',e=>{if(e.target===onlineSheet)onlineSheet.hidden=true;});
  ...
  async function searchOnline(term){
    ...
    try{
      ...
      const data=await r.json();const products=(data.products||[]).filter(p=>p.product_name&&p.code).slice(0,10);
      ...
      $('onlineList').querySelectorAll('[data-online]').forEach(b=>b.addEventListener('click',()=>{
        const p=products[Number(b.dataset.online)];onlineSheet.hidden=true;clickTab('foods');
        setTimeout(()=>{const code=$('barcodeInput');if(code){code.value=String(p.code);$('lookupBarcodeBtn')?.click();document.querySelector('.custom-food-card')?.scrollIntoView({behavior:'smooth',block:'start'});}},100);
      }));
  ...
  const shelf=document.createElement('section');shelf.className='quick-shelf';
  quickAdd.insertAdjacentElement('beforebegin',shelf);
  const favBtn=document.createElement('button');favBtn.type='button';favBtn.className='fav-toggle';favBtn.id='favoriteCurrentBtn';favBtn.setAttribute('aria-label','Add current food to favourites');favBtn.textContent='☆';
  const quickHeading=quickAdd.querySelector('.section-heading');quickHeading?.appendChild(favBtn);

  function stateWithFavs(){const s=readState();if(!Array.isArray(s.favorites))s.favorites=[];return s;}
  function toggleFavorite(){const id=foodSelect.value;if(!id)return;const s=stateWithFavs();const i=s.favorites.indexOf(id);if(i>=0)s.favorites.splice(i,1);else s.favorites.unshift(id);writeState(s);syncFavorite();renderShelf();}
  function syncFavorite(){const s=stateWithFavs();const yes=s.favorites.includes(foodSelect.value);favBtn.textContent=yes?'★':'☆';favBtn.setAttribute('aria-label',yes?'Remove current food from favourites':'Add current food to favourites');}
  favBtn.addEventListener('click',toggleFavorite);foodSelect.addEventListener('change',syncFavorite);

  function recentIds(state){
    const rows=[];Object.keys(state.logs||{}).sort().reverse().forEach(day=>{(state.logs[day]||[]).slice().reverse().forEach(x=>rows.push(x));});
    const seen=new Set(),out=[];for(const x of rows){const id=x.foodId;if(id&&!seen.has(id)){seen.add(id);out.push(id);}if(out.length>=8)break;}return out;
  }
  ...
  function renderShelf(){
    const s=stateWithFavs();const favs=s.favorites.filter(id=>Array.from(foodSelect.options).some(o=>o.value===id)).slice(0,6);const recent=recentIds(s).filter(id=>!favs.includes(id)).slice(0,6);
    const content=[...favs.map(id=>chip(id,'Favourite')),...recent.map(id=>chip(id,'Recent'))].join('');
    shelf.innerHTML=`<div class="quick-shelf-head"><h3>Quick picks</h3><span>${favs.length?'Favourites + recent':'Recent foods appear here'}</span></div><div class="quick-chips">${content||'<span style="color:var(--muted);font-size:.82rem">Log a few foods and your quickest choices will appear here.</span>'}</div>`;
    shelf.querySelectorAll('[data-quick-food]').forEach(b=>b.addEventListener('click',()=>selectFood(b.dataset.quickFood)));
  }
  ...
  // Make the main Foods search more forgiving by handing unmatched terms to the global search.
