...
const calcFood=(food,grams)=>({
  kcal:round((Number(food.kcal)||0)*Number(grams)/100),
  protein:round((Number(food.protein)||0)*Number(grams)/100),
  fibre:round((Number(food.fibre)||0)*Number(grams)/100)
});
...
(function testCalibratedGoatBecomesActionablePieces(){
  ...
  for(const [i,grams] of [55.5,56.5,57.5].entries()){
  ...
  assert.equal(out.pieceSuggestion.estimateBasisGrams,56.5);
  assert.equal(out.pieceSuggestion.estimateSource,'personal-piece-weight');
  assert.equal(out.pieceSuggestion.estimatedGrams,169.5);
  ...
  const actionableExpected=calcFood(food,out.grams);
  const targetExpected=calcFood(food,out.targetGrams);
  assert.equal(log.grams,out.grams);
  assert.equal(log.estimatedGrams,out.grams);
  assert.ok(Math.abs(log.kcal-actionableExpected.kcal)<1e-9,'logged kcal must derive from actionable piece grams');
  assert.ok(Math.abs(log.protein-actionableExpected.protein)<1e-9,'logged protein must derive from actionable piece grams');
  assert.notEqual(log.kcal,targetExpected.kcal,'logged kcal incorrectly used targetGrams');
})();
