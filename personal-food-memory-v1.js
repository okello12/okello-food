(() => {
  ...
  function profile(id){
    ...
    if(!rows.length)return {id:String(id),count:0,confidence:'new',usualGrams:null,mealUsual:{},preferredMeal:null,lastSeen:null,recentCount:0,satiety:{hungry:0,comfortable:0,full:0},comfortableRate:null};
    const sat=satiety();
    ...
    for(const meal of ['Lunch','Dinner','Snack','Other']){
      const vals=rows.filter(x=>x.meal===meal).map(x=>x.grams);
    ...
    return {
      ...
      usualGrams:round1(median((recent.length>=3?recent:rows).map(x=>x.grams))),
      mealUsual,
  ...
  function usualPortion(id,meal){const p=profile(id);if(p.count<3)return null;return p.mealUsual[meal]||p.usualGrams||null;}
  ...
  if(foodSelect&&mealSelect&&gramsInput&&quick&&!$('personalMemoryStrip')){
    ...
    function renderStrip(){
      ...
      strip.innerHTML=`<div><strong>Your usual logged amount: ${Math.round(usual)} g</strong><small>${p.count} logs · ${mealText}${p.comfortableRate!=null?` · comfortable ${Math.round(p.comfortableRate)}% of rated meals`:''}</small></div><button id="memoryUseUsual" class="memory-use" type="button">Use my usual</button>`;
      strip.hidden=false;
    ...
    foodSelect.addEventListener('change',renderStrip);mealSelect.addEventListener('change',renderStrip);renderStrip();
  }
  ...
  if(settings&&!$('personalFoodMemoryCard')){
    ...
    const rows=learned.map(x=>`<div class="memory-row"><div><strong>${String(x.f.emoji||'🍽️')} ${String(x.f.name||'')}</strong><small>${x.p.preferredMeal?`Usually ${x.p.preferredMeal.toLowerCase()} · `:''}${x.p.count} logs</small></div><b>${Math.round(x.p.usualGrams||x.f.portion||0)} g</b></div>`).join('');
    card.innerHTML=`<p class="eyebrow">PERSONAL FOOD MEMORY</p><h3>Learning how you actually eat</h3><p class="muted">Okello now learns your usual portions, common foods, preferred meal times and satiety feedback on this device. It can use that memory to break close search ties and surface your normal amount without overriding the calorie target.</p><div class="memory-list">${rows||'<div class="tiny-note">Log the same foods a few times and learned portions will appear here.</div>'}</div><p class="tiny-note">Personal memory stays in local browser storage. It is not uploaded to GitHub.</p>`;
