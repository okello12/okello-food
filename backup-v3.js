(() => {
  ...
  const STORES=Object.freeze([
    {key:MAIN_STORE,field:'state',type:'object',durability:'durable'},
  ...
  function completeBundle(){
    const out={format:PLAIN_FORMAT,version:VERSION,exportedAt:new Date().toISOString(),storageModel:'local-device',manifest:{durableStores:STORES.filter(x=>x.durability==='durable').map(x=>x.key),draftStores:STORES.filter(x=>x.durability==='draft').map(x=>x.key),deliberatelyExcluded:[...DISPOSABLE]}};
    for(const spec of STORES){const value=parseStore(spec.key);if(value!==null||spec.field==='state')out[spec.field]=value;}
  ...
  function downloadText(text,name,type='application/json'){const blob=new Blob([text],{type});const anchor=document.createElement('a');anchor.href=URL.createObjectURL(blob);anchor.download=name;document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(anchor.href),1500);}
  ...
  async function deriveKey(pass,salt){const material=await crypto.subtle.importKey('raw',new TextEncoder().encode(pass),'PBKDF2',false,['deriveKey']);return crypto.subtle.deriveKey({name:'PBKDF2',salt,iterations:180000,hash:'SHA-256'},material,{name:'AES-GCM',length:256},false,['encrypt','decrypt']);}
  async function exportPlain(){const payload=completeBundle();downloadText(JSON.stringify(payload,null,2),`okello-food-complete-backup-${todayKey()}.json`);toast('Complete v3 backup created');}
  async function importPlain(file){const parsed=validateBundle(JSON.parse(await file.text()));if(!confirmRestore(parsed)){toast('Restore cancelled');return false;}const result=restoreBundle(parsed);sessionStorage.setItem('okello_flash',result.legacy?'Older backup restored safely':'Complete backup restored safely');location.reload();return true;}
  ...
  function updateCopy(){
    ...
    const secure=$('encryptedExportBtn')?.closest('.secure-transfer');const secureNote=secure?.querySelector('.secure-note');if(secureNote)secureNote.textContent='Encrypted backups use your passphrase locally. Scanner diagnostics, temporary runtime traces and touch-debug counters are deliberately excluded.';
  }
  updateCopy();
