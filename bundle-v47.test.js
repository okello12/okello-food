...
assert.ok(index.includes('bootstrap-v14.js?v=47'),'index bootstrap query is not v47');
assert.ok(bootstrap.includes("const VERSION = '47'"),'bootstrap version is not 47');
assert.ok(bootstrap.includes("service-worker.js?v=47"),'bootstrap service-worker registration is not v47');
assert.ok(sw.includes("const VERSION='47'"),'service-worker version is not 47');
assert.ok(sw.includes("const CACHE='okello-food-v47'"),'service-worker cache name is not v47');
...
assert.equal(scripts.length,58,'v47 should boot exactly 58 runtime scripts');
for(const script of scripts)assert.ok(sw.includes(`'./${script}'`),`service worker does not cache bootstrap script ${script}`);

const cachedScripts=[...sw.matchAll(/'\.\/([^']+\.js\?v=47)'/g)].map(m=>m[1]).filter(name=>name!=='bootstrap-v14.js?v=47');
...
before('target-safety-v46.js?v=47','global-first-v47.js?v=47');
...
for(const file of ['global-first-v47.js','backup-v3.js'])assert.ok(fs.existsSync(path.join(__dirname,file)),`${file} is missing from repository`);
